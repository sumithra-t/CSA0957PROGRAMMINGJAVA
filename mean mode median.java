...
import java.util.*;
    ...
    {
	int num_value,y,g,asce;
	Scanner sc=new Scanner(System.in);
	System.out.print("Enter the number of elements: ");
      num_value=sc.nextInt();
	int[] invalue = new int[num_value];
	System.out.println("Enter the elements: ");
	for(y=0;y<num_value;y++)
	{
		    invalue[y]=sc.nextInt();
	}
	for(y=0;y<num_value;y++)
	{
		for(g=y+1;g<num_value;++g)
		{
		       if(invalue[y]>invalue[g])
		      {
		            asce=invalue[y];
		            invalue[y]=invalue[g];
		      	invalue[g]=asce;
